...
'''
หลักของ problem set นี้คือการฝึก knowledge engineering

เราจะทำการแปลงความหมายของโจทย์ลงไปยัง knowledge base "แบบตรงๆ" ตามที่สื่อ

ซึ่งทุกอย่างที่เราใส่ลงไป'ต้องเป็นความจริง!' 

เช่น Knight ต้องพูดจริงเสมอ
Knave ต้องพูดปดเสมอ

เพื่อที่จะได้หาในโปรแกรมว่า

Kb ของเรา entail symbol ตัวไหน (หมายความว่า

symbol ตัวไหนที่จะเป็น True  เมื่อ KB ที่เราออกแบบ true หมด


แบ่งโครงสร้างของ KB ออกเป็นสองส่วน 

1. fact ของ เกมส์ knight and knave
เช่น ตัวละครจะเป็นทั้ง สองบทบาทไม่ได้

2. message ที่ โจทย์ให้มา แปลงให้เป็นกรณีความจริงต่างๆ เช่น

ถ้า A พูดว่า .....
ก็หมายความว่า
A เป็น Knight แสดงว่า ที่พูดมา 'เป็นความจริง'
A เป็น Knave แสดงว่า ที่พูดมา 'เป็นเท็จ

และทั้งกรณีข้างต้นต้อง 'จริง' เสมอ
...
knowledge0 = And(
    ...
    # ถ้า A เป็น Knave A จะพูดเท็จ หมายความว่าประโยต And(AKnight,AKnave) = False
    # ประโยค   (ถ้า A เป็น Knave A จะพูดเท็จ) ต้องเป็นจริง

    # Implication(AKnave,Not(And(AKnight,AKnave))) = T
    #               T --->      T

    # Not(And(AKnight,AKnave)) = T
    #  And(AKnight,AKnave) = F
